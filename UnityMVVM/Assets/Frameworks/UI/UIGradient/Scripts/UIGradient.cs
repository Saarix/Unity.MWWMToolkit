...
namespace JoshH.UI
{
    ...
    public class UIGradient : BaseMeshEffect
    {
            ...
            if (this.enabled)
            {
                ...
                for (int i = 0; i < vh.currentVertCount; i++)
                {
                    ...
                    if (size != Vector2.zero)
                        normalizedPosition = ((Vector2)vert.position - rectTransform.rect.min) / size;
#else
                    Vector2 size = rectTransform.rect.max - rectTransform.rect.min;
                    ...
                    if (normalizedPosition.x is float.NaN)
                        normalizedPosition = new Vector2(0, normalizedPosition.y);
                    if (normalizedPosition.y is float.NaN)
        ...
        protected void CutMesh(VertexHelper vh)
        {
            ...
            Vector2 d = GetCutDirection();

            IEnumerable<float> cuts = linearGradient.alphaKeys.Select(x => x.time);
            cuts = cuts.Union(linearGradient.colorKeys.Select(x => x.time));

            foreach (float item in cuts)
            {
                list.Clear();
                Vector2 point = GetCutOrigin(item);
